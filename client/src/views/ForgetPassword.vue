  ...
  <div class="flex items-center justify-center min-h-screen p-6">
    <div class="w-full h-full max-w-sm min-w-[350px] p-6 lg:bg-white lg:rounded-lg lg:shadow-lg lg:p-8 dark:lg:bg-darkPrimary-700 rounded-lg dark:bg-darkPrimary-800">
      ...
      <VForm @submit="forgetPassword">
        ...
        <button v-if="isLoading" class="flex items-center justify-center w-full px-4 py-2 rounded-md bg-primary-600 dark:bg-indigo-600 text-neutral-50 disabled:pointer-events-none disabled:opacity-50" disabled="">
          <div class="w-4 h-4 border-2 border-white border-solid rounded-full animate-spin border-t-transparent"></div>
          <span class="ml-2">Loading... </span>
        </button>

        <button v-else type="submit" class="flex items-center justify-center w-full px-4 py-2 text-white rounded-md bg-primary-600 dark:bg-indigo-600 hover:dark:bg-indigo-700 hover:bg-primary-700 outline-1 focus:outline-2 focus:outline-primary-500 focus:outline-offset-2 focus:outline-none">送出</button>
