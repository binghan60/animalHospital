    ...
    <div class="w-full h-full max-w-sm min-w-[330px] p-6 lg:bg-white lg:rounded-lg lg:shadow-lg lg:p-8 dark:lg:bg-darkPrimary-700 rounded-lg dark:bg-darkPrimary-800">
      ...
      <VForm @submit="register">
        ...
        <div v-show="registerForm.role === 'user'" class="mb-4">
          <label for="nickname" class="text-primary-900 dark:text-darkPrimary-50">暱稱*</label>
          <VField id="nickname" v-model="registerForm.nickname" rules="required|length:1,20" type="text" name="nickname" class="w-full h-8 pl-3 mt-2 rounded-md shadow-sm dark:bg-darkPrimary-500 dark:text-darkPrimary-50 text-primary-900 outline-1 outline-primary-100 focus:outline-2 focus:outline-primary-400 dark:placeholder-darkPrimary-400 dark:focus:outline-darkPrimary-400 focus:outline-none" placeholder="請輸入暱稱" autocomplete="off" />
          <ErrorMessage class="mt-1 text-sm text-red-600 dark:text-rose-400" name="nickname" />
        </div>
